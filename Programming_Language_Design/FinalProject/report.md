...
- 由於檢查及更改值時只會給予用戶名稱及工作表名稱，本實作讓同一個用戶無法存取到有重複名稱的工作表。（在創建及分享時不允許重複）
- 工作表的權限對所有能存取到它的用戶都是相同的，並且只有擁有者能設定。（此為我對作業要求的理解）
- 更改數值時只支援二元運算。（因為scala沒有eval)
### 1.2 定義的資料結構
- `User`：表示用戶，包含用戶名、擁有的工作表、共享的工作表。
...
- 共享控制：在確認用戶是否能存取某個工作表時會使用`sharingControl.hasAccess(sheet, userName)`。而在執行`shareSheet`時會使用`sharingControl.sharingEnabled`來確認功能是否開啟。
...
#### 6.1-1 同個名字的Sheet只能有一張
  - 如果`UserA`，`UserB`先後創建了各自的Sheet且都命名為`MySheet`, 那程式會不知道從命令列輸入的指令所指的`MySheet`是指誰的`MySheet`。
